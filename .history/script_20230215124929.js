...
window.onload = function(){
    ...
    var canvasHeight = 450;//Dimension du canvas hauteur
    ...
    init();//on appelle la fonction pour afficher la page
     ...
     //la fonction init est un standard pour initialiser les choses
    function init(){
    ...
    function drawBlock(ctx, position){// position = position d'un block, on parle de block par facilité mais ce sont plutôt des pixels
        ...
        ctx.fillRect(x,y,blockSize,blockSize);//remplir le serpent et prendra la taille du bloc qui fait 30px
    }// 1 block = 30px
    ...
    function Snake(body, direction){//toujours mettre une majuscule au début d'une fonction objet, body = création du corps du serpent
        ...
        this.setDirection = function(newDirection){ //Passer la nouvelle direction au serpent
         ...
         //fonction lorsqu'il y a colision (prendre un mur ou il se touche la queue)
     ...
     //Création de la pomme (Constructeur de la pomme)
    function Apple(position){//fonction pour dessiner la pomme
        ...
        this.draw = function(){
          ...
          ctx.beginPath();
          var radius = blockSize/2;//rayon, taille d'un bloc /2
          var x = this.position[0]*blockSize + radius;//position
         ...
         //Donner une nouvelle position à la pomme lorsque le serpent l'à manger
