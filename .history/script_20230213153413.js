    ...
    function Snake(body) //toujours mettre une majuscule au début d'une fonction objet, body = création du corps du serpent
     {
        ...
        this.draw = function()//dessiner le corps du serpent ds notre canvas
        {
            ...
            ctx.fillStyle = "rgba(xff0000)";//couleur du serpent
            for(var i = 0; i < this.body.length; i++);// on crée une boucle pour pouvoir déssiner notre serpent et pouvoir se déplacer par la suite ds le body
               {
                drawBlock(ctx, this.body)//la fonction drawBlock permettra de dessiner un bloc, donner ds le context (ctx) ds lequel le canvas où on veut dessiner.
                           //On veut pouvoir passer ds chacuns des blocks du body (this.)
               } 
            
            [[6,4], [5,4], [4,4]];//on doit maintenant créer un tableau et ds ce tableau il y a X cellules. 
