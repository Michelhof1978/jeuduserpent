...
window.onload = function(){      //fonction js onload, va permettre de lancer la fenêtre créee lorque la page s'affiche
   var canvasWidth = 900;
    ...
    function drawScore(){       //Afficher le score à l'écran
        ctx.save();     //Commentaire Idem que pour la fonction game over
        ...
        var centreX = canvasWidth / 2;
        var centreY = canvasHeight / 2;
        ctx.fillText(score.toString(), centreX, centreY);       //Avec fillText, il faudra que le score qui est number soit en string 
        ctx.restore();
    }
